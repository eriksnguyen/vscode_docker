...
from aiflow.ext.dataclasses import ReplaceMixin, SerializeMixin
...
def test_replace_mixin():
    ...
    class Bar(Foo, ReplaceMixin):
        """`Bar` takes `Foo` and adds `Replaceable` mechanics"""

    assert Bar(x="x", y=0, z="z").replace(x="xx") == Bar(x="xx", y=0, z="z")
